<script lang="ts">
	...
	let { syncProgress, syncResult, isSyncing, pendingCount }: Props = $props();
	...
	const showBar = $derived(!!statusText || pendingCount > 0);
</script>
...
{#if showBar}
	<div class="flex items-center gap-2 border-t border-surface-200-800 px-4 py-1 text-xs text-surface-500-400 bg-surface-50-950 shrink-0">
		...
		{#if pendingCount > 0}
			<span class="text-warning-500">{statusText ? "·" : ""} {pendingCount} pending</span>
		{/if}
	</div>
{/if}
